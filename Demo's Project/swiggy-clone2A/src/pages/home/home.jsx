    ...
    <div className="home-container">
      <div className="header-section">
        <Header
          ...
          lat={lat}
          lon={lon}
          changeHeader={changeHeaderContent}
        />
      </div>
        ...
        {!isLoading &&
          ...
          data.data.cards.map((card) => {
            switch (card.card.card.id) {
              case "topical_banner":
                return (
                  <BestOffer key={card.card.card.id} data={card.card.card} />
                );
              case "whats_on_your_mind":
                return (
                  <FoodItems key={card.card.card.id} data={card.card.card} />
                );
              ...
              case "restaurant_grid_listing":
                return (
                  <Restaurants key={card.card.card.id} data={card.card.card} />
                );
              case "restaurant_near_me_links":
              case "popular_cities":
                return (
                  <NearMeLinks key={card.card.card.id} data={card.card.card} />
                );
              //   case "swiggy_not_present":
              //     return (
              //       <LocationUnservicable
              //         key={card.card.card.id}
              //         data={card.card.card}
              //       />
              //     );
              case "popular_restaurants_title":
                ...
                console.log(
                  "Title: ",
                  card.card && card.card.card && card.card.card.title
                );
                return;
      ...
      {MASTER.data.cards.map((card) => {
        switch (card.card.card.id) {
          case "app_install_links":
            return (
              <MobileDownload key={card.card.card.id} data={card.card.card} />
            );
